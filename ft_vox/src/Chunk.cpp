...
{
	_blocks = std::make_unique<octree::OctreeNode>(glm::vec3(pos.x, 32.0f, pos.y), 32.0f);
...
auto Chunk::at(std::size_t x, std::size_t y, std::size_t z) const -> Block
{
	...
	return _blocks->at_local((int)x, (int)y, (int)z).value_or(_void);
}
...
void Chunk::setBlock(size_t x, size_t y, size_t z, Block val)
{
	...
	_blocks->set_local((int)x, (int)y, (int)z, val);
}
